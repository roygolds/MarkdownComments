...
- Comments sidebar: an activity-bar `WebviewView` that shows only the active Markdown document's comment threads as Word-style side cards (no Markdown body) and offers the same reply/edit/resolve/reopen/delete actions plus collapse-all and hide-resolved toggles. Unlike the panel (pinned to one document), the sidebar follows the active Markdown editor and the interactive preview panel (via an active-preview event) and re-renders live (debounced) on document changes. It shares the panel's webview script, styles, security posture, and the common `CommentEditController`. Clicking a comment card posts a validated, uri-guarded `reveal` message; if the interactive preview panel is currently open for that document, the sidebar routes the reveal to the panel (which is brought forward and posts a `revealThread` message so the webview scrolls to the thread element and briefly highlights it), keeping the user in the preview instead of the raw Markdown. Otherwise it reveals the thread's anchored range in a text editor: if an editor for the document is already visible it scrolls/selects the line in place without stealing focus, and if VS Code's built-in Markdown preview is the active tab it opens the source editor beside the preview with focus preserved. VS Code exposes no API to scroll the built-in preview directly, so this relies on editor->preview scroll-sync (`markdown.preview.scrollPreviewWithEditor`, on by default) to bring the preview to the commented line. While the sidebar is visible, the read-only built-in Markdown preview suppresses its inline comment cards (the markdown-it plugin returns nothing for comment fences) so comments appear only in the sidebar; the built-in preview is re-rendered via `markdown.preview.refresh` (which cleans the markdown engine cache and refreshes open previews so the suppression takes effect live — `markdown.api.reloadPlugins` is only a fallback because it reloads plugin code without refreshing already-open previews). Limitation: VS Code's built-in Markdown preview tab exposes no source-document URI, so when such a preview tab is focused the sidebar keeps the last Markdown editor target rather than re-resolving it. Limitation: VS Code's built-in Markdown preview tab exposes no source-document URI, so when such a preview tab is focused the sidebar keeps the last Markdown editor target rather than re-resolving it.
